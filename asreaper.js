...
///MAİN DE YUKARIYA KOYUN


client.on("ready", () => {
  client.channels.cache.get("839855831670980698").join();
});
...
client.on("guildMemberAdd", member => {
 ...
 //
  .setDescription(` **Heyy , Hoş Geldin!** ${
        member.user
      }, Seninle Beraber Toplam **${
        guild.memberCount
      }** Kişi Olduk! \n  Kaydının Yapılması İçin **İsim** ve **Yaş** Yazman Yeterli . \n  Hesabının kuruluş tarihi: **${moment(
        user.createdAt
      ).format("DD")} ${aylar[moment(user.createdAt).format("MM")]} ${moment(
        user.createdAt
      ).format(
        "YYYY HH:mm:ss"
       )}** \n  Bu Üyenin Hesabı: ${kontrol} \n  <@&${kayıtçı}> Rolündeki Yetkililer Hemen İlgileneceklerdir.!`);
  //
  client.channels.cache.get(kanal).send(embed);
  client.channels.cache.get(kanal).send(` **ŞŞŞ Kaydet Adamı** <@&${kayıtçı}>`);
});
...
client.on("message", async msg => {
    ...
    if(
      msg.content.toLowerCase() == "sa"//-- KİŞİNİN YAZDIĞI 
       ...
       return msg.channel.send("**Aleyküm Selam, Hoşgeldin :heart: ** "); ///- BOTUN VERDİĞİ CEVAP
    }
...
client.on("message", async msg => {
    ...
    if(
      msg.content.toLowerCase() == "taşşaklı"//-- KİŞİNİN YAZDIĞI 
      
    ) {
       return msg.channel.send("**da kalk siktir git taşşaklı diyor ** "); ///- BOTUN VERDİĞİ CEVAP
    }
...
client.on("message", async msg => {
    ...
    if(
      msg.content.toLowerCase() == "la fearless"//-- KİŞİNİN YAZDIĞI 
      
    ) {
       return msg.channel.send("**şşş aloo , bana la deme s1kerim belanı ** "); ///- BOTUN VERDİĞİ CEVAP
    }
...
client.on("message", async msg => {
    ...
    if(
      msg.content.toLowerCase() == "şş fearless"//-- KİŞİNİN YAZDIĞI 
      
    ) {
       return msg.channel.send("**Efendim  Canını Yediğimm ?** "); ///- BOTUN VERDİĞİ CEVAP
    }
...
client.on("message", async msg => {
    ...
    if(
      msg.content.toLowerCase() == "fearless"//-- KİŞİNİN YAZDIĞI 
      
    ) {
       return msg.channel.send("**Efendim , Gardeşim Nasıl Yardımcı Olayım ** "); ///- BOTUN VERDİĞİ CEVAP
    }
...
client.on("message", async msg => {
    ...
    if(
      msg.content.toLowerCase() == "sg"//-- KİŞİNİN YAZDIĞI 
      
    ) {
       return msg.channel.send("**kes kafana şamar atarım ** "); ///- BOTUN VERDİĞİ CEVAP
    }
...
client.on("message", async msg => {
    ...
    if(
      msg.content.toLowerCase() == "31"//-- KİŞİNİN YAZDIĞI 
       ...
       return msg.channel.send("**sen çok komiksin aq xd ** "); ///- BOTUN VERDİĞİ CEVAP
    }
...
client.on("message", async msg => {
    ...
    if(
      msg.content.toLowerCase() == "naber"//-- KİŞİNİN YAZDIĞI 
      
    ) {
       return msg.channel.send("**iyiyim sen ? :heart: ** "); ///- BOTUN VERDİĞİ CEVAP
    }
...
client.on("message", async msg => {
    ...
    if(
      msg.content.toLowerCase() == "seni seviyorum"//-- KİŞİNİN YAZDIĞI 
      
    ) {
       return msg.channel.send("**Bende Seni Seviyorum Ühüüü :heart: :) ** "); ///- BOTUN VERDİĞİ CEVAP
    }
...
client.on("message", async msg => {
    ...
    if(
      msg.content.toLowerCase() == "selam"//-- KİŞİNİN YAZDIĞI 
...
client.on("message", async msg => {
    ...
    if(
      msg.content.toLowerCase() == "selamun aleyküm"//-- KİŞİNİN YAZDIĞI 
